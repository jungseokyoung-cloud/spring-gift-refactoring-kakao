...
description: >
  ...
  TEST_STRATEGY.md의 검증 행위 목록과 시나리오 매트릭스를 기반으로
...
10. 검증 서브에이전트 실행 (아래 "자동 검증 루프" 참조)
11. 위반 사항 있으면 테스트 코드 수정 후 ./gradlew test 재실행
12. 검증 통과 또는 최대 3회 반복 후 종료
```

## 자동 검증 루프

Step 9까지 완료 후, 생성된 테스트 코드가 이 스킬의 모든 요구사항을 충족하는지 자동 검증한다.

### 검증 서브에이전트 호출

Task 도구로 `Explore` 타입 서브에이전트를 호출한다. 프롬프트 템플릿:

```
아래 두 파일을 읽고, 체크리스트의 모든 항목에 대해 테스트 코드를 검증하라.

- 체크리스트: .claude/skills/acceptance-test-writer/VALIDATION_CHECKLIST.md
- 테스트 파일: {생성된 테스트 파일 경로}

판정 결과를 아래 JSON 스키마로만 반환하라. 다른 텍스트, 설명, 마크다운을 포함하지 마라.
...
판정 규칙:
- MUST 위반 ≥ 1 → pass=false
- MUST 위반 0, SHOULD 위반 ≥ 1 → pass=false
- 위반 0건 → pass=true
- 해당 테스트에 적용되지 않는 항목은 skipped로 처리하고 violations에 포함하지 마라
- stats.total = stats.passed + stats.violated + stats.skipped 이어야 한다
```

### 피드백 루프

```
[Step 9 완료: ./gradlew test 통과]
        │
        ▼
[Step 10: Task로 검증 서브에이전트 호출]
        │
   ┌────┴────┐
   │         │
pass=true  pass=false
   │         │
   ▼         ▼
 완료    [Step 11: violations 기반으로 테스트 코드 수정 + ./gradlew test]
              │
              ▼
         반복 횟수 < 3? ──yes──→ Step 10으로
              │
             no
              ▼
         잔여 위반 사항을 사용자에게 보고 후 종료
```

### 피드백 루프 규칙
- **최대 반복 횟수**: 3회 (무한 루프 방지)
- **비정상 출력 처리**: 서브에이전트 응답이 JSON 파싱 실패 또는 필수 필드(`pass`, `violations`, `stats`, `summary`) 누락 시, 원문을 사용자에게 출력하고 검증을 스킵한다
- **테스트 파일 미생성**: 검증 대상 파일이 없으면 검증 단계를 스킵한다
- **금지 사항 충돌**: 서브에이전트의 suggestion이 BAN 항목과 충돌하면 BAN 규칙을 우선한다
- **수정 범위**: 검증 루프 중에도 `src/test/` 경로만 수정 가능 (PATH-1, PATH-2 준수)
